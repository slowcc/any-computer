...
import { apiClient, authAtom } from "./client";
export const wagmiConfig = createConfig(
  getDefaultConfig({
    // Your dApp's info
    appName: "Slow Computer",
    // Optional links
    appDescription: "Sign in with Ethereum",
    appUrl: "https://slow.land",
    // Chains
    chains: [mainnet],
...
export const siweConfig: SIWEConfig = {
  getNonce: async () => {
    const response = await apiClient.get("/siwe/nonce");
  ...
  verifyMessage: async ({ message, signature }) => {
    try {
      const { success, token, address } = await apiClient.post("/siwe/verify", {
        message,
        signature,
      });

      const store = getDefaultStore();
      store.set(authAtom, { isAuthenticated: true, user: { address }, token });
  ...
  getSession: async () => {
    try {
      return await apiClient.get("/siwe/session");
    } catch (error) {
      return null;
    }
  },
  ...
  signOut: async () => {
    try {
      const store = getDefaultStore();
      // Reset both auth and wallet address atoms
      store.set(authAtom, { isAuthenticated: false, user: null, token: null });

      await apiClient.post("/siwe/logout");
      return true;
    } catch (error) {
      return false;
    }
...
export const connectKitTheme = {
  ...
  "--ck-tertiary-button-border-radius": "4px",
